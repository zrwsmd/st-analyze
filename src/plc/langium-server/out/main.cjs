...
`],tracer:(y,T)=>T()});let r=e.tracer;r("initCharCodeToOptimizedIndexMap",()=>{fB()});let n;r("Reject Lexer.NA",()=>{n=to(t,y=>y[wo]===Dt.NA)});let i=!1,o;r("Transform Patterns",()=>{i=!1,o=K(n,y=>{let T=y[wo];if(fn(T)){let k=T.source;return k.length===1&&k!=="^"&&k!=="$"&&k!=="."&&!T.ignoreCase?k:k.length===2&&k[0]==="\\"&&!ut(["d","D","s","S","t","r","n","t","0","c","b","B","f","v","w","W"],k[1])?k[1]:e.useSticky?HE(T):qE(T)}else{if(Nr(T))return i=!0,{exec:T};if(typeof T=="object")return i=!0,T;if(typeof T=="string"){if(T.length===1)return T;{let k=T.replace(/[\\^$.*+?()[\]{}|]/g,"\\$&"),B=new RegExp(k);return e.useSticky?HE(B):qE(B)}}else throw Error("non exhaustive match")}})});let s,a,l,u,c;r("misc mapping",()=>{s=K(n,y=>y.tokenTypeIdx),a=K(n,y=>{let T=y.GROUP;if(T!==Dt.SKIPPED){if(zt(T))return T;if(gr(T))return!1;throw Error("non exhaustive match")}}),l=K(n,y=>{let T=y.LONGER_ALT;if(T)return ie(T)?K(T,B=>Vf(n,B)):[Vf(n,T)]}),u=K(n,y=>y.PUSH_MODE),c=K(n,y=>ee(y,"POP_MODE"))});let f;r("Line Terminator Handling",()=>{let y=tS(e.lineTerminatorCharacters);f=K(n,T=>!1),e.positionTracking!=="onlyOffset"&&(f=K(n,T=>ee(T,"LINE_BREAKS")?!!T.LINE_BREAKS:eS(T,y)===!1&&Yf(y,T.PATTERN)))});let d,h,_,x;r("Misc Mapping #2",()=>{d=K(n,QE),h=K(o,uB),_=Rt(n,(y,T)=>{let k=T.GROUP;return zt(k)&&k!==Dt.SKIPPED&&(y[k]=[]),y},{}),x=K(o,(y,T)=>({pattern:o[T],longerAlt:l[T],canLineTerminator:f[T],isCustom:d[T],short:h[T],group:a[T],push:u[T],pop:c[T],tokenTypeIdx:s[T],tokenType:n[T]}))});let E=!0,I=[];return e.safeMode||r("First Char Optimization",()=>{I=Rt(n,(y,T,k)=>{if(typeof T.PATTERN=="string"){let B=T.PATTERN.charCodeAt(0),Q=qn(B);TT(y,Q,x[k])}else if(ie(T.START_CHARS_HINT)){let B;J(T.START_CHARS_HINT,Q=>{let ae=typeof Q=="string"?Q.charCodeAt(0):Q,le=qn(ae);B!==le&&(B=le,TT(y,le,x[k]))})}else if(fn(T.PATTERN))if(T.PATTERN.unicode)E=!1,e.ensureOptimizations&&fa(`${El}	Unable to analyze < ${T.PATTERN.toString()} > pattern.
	...
	See https://chevrotain.io/docs/guide/resolving_lexer_errors.html#ANCHORS	for details.`,type:ct.SOI_ANCHOR_FOUND,tokenTypes:[i]}))}function rB(t){let e=Zt(t,n=>{let i=n[wo];return i instanceof RegExp&&(i.multiline||i.global)});return K(e,n=>({message:"Token Type: ->"+n.name+"<- static 'PATTERN' may NOT contain global('g') or multiline('m')",type:ct.UNSUPPORTED_FLAGS_FOUND,tokenTypes:[n]}))}function nB(t){let e=[],r=K(t,o=>Rt(t,(s,a)=>(o.PATTERN.source===a.PATTERN.source&&!ut(e,a)&&a.PATTERN!==Dt.NA&&(e.push(a),s.push(a)),s),[]));r=Gn(r);let n=Zt(r,o=>o.length>1);return K(n,o=>{let s=K(o,l=>l.name);return{message:`The same RegExp pattern ->${er(o).PATTERN}<-has been used in all of the following Token Types: ${s.join(", ")} <-`,type:ct.DUPLICATE_PATTERNS_FOUND,tokenTypes:o}})}function iB(t){let e=Zt(t,n=>{if(!ee(n,"GROUP"))return!1;let i=n.GROUP;return i!==Dt.SKIPPED&&i!==Dt.NA&&!zt(i)});return K(e,n=>({message:"Token Type: ->"+n.name+"<- static 'GROUP' can only be Lexer.SKIPPED/Lexer.NA/A String",type:ct.INVALID_GROUP_TYPE_FOUND,tokenTypes:[n]}))}function oB(t,e){let r=Zt(t,i=>i.PUSH_MODE!==void 0&&!ut(e,i.PUSH_MODE));return K(r,i=>({message:`Token Type: ->${i.name}<- static 'PUSH_MODE' value cannot refer to a Lexer Mode ->${i.PUSH_MODE}<-which does not exist`,type:ct.PUSH_MODE_DOES_NOT_EXIST,tokenTypes:[i]}))}function sB(t){let e=[],r=Rt(t,(n,i,o)=>{let s=i.PATTERN;return s===Dt.NA||(zt(s)?n.push({str:s,idx:o,tokenType:i}):fn(s)&&lB(s)&&n.push({str:s.source,idx:o,tokenType:i})),n},[]);return J(t,(n,i)=>{J(r,({str:o,idx:s,tokenType:a})=>{if(i<s&&aB(o,n.PATTERN)){let l=`Token: ->${a.name}<- can never be matched.
