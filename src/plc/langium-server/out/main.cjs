...
`),B.log(`Received request '${R.method} - (${R.id})'.`,b)}else zr("receive-request",R)}function xi(R){if(!(T===Pe.Off||!B||R.method===qu.type.method))if(k===Yr.Text){let b;(T===Pe.Verbose||T===Pe.Compact)&&(R.params?b=`Params: ${mt(R.params)}
...
See https://chevrotain.io/docs/guide/resolving_lexer_errors.html#UNREACHABLE`;e.push({message:l,type:ct.UNREACHABLE_PATTERN,tokenTypes:[n,a]})}})}),e}function aB(t,e){if(fn(e)){let r=e.exec(t);return r!==null&&r.index===0}else{if(Nr(e))return e(t,0,[],{});if(ee(e,"exec"))return e.exec(t,0,[],{});if(typeof e=="string")return e===t;throw Error("non exhaustive match")}}function lB(t){return qn([".","\\","[","]","|","^","$","(",")","?","*","+","{"],r=>t.source.indexOf(r)!==-1)===void 0}function jE(t){let e=t.ignoreCase?"i":"";return new RegExp(`^(?:${t.source})`,e)}function HE(t){let e=t.ignoreCase?"iy":"y";return new RegExp(`${t.source}`,e)}function YE(t,e,r){let n=[];return ee(t,ha)||n.push({message:"A MultiMode Lexer cannot be initialized without a <"+ha+`> property in its definition
...
inside top level rule: ->`+t.name+"<-"}},On={buildDuplicateFoundError(t,e){function r(c){return c instanceof _e?c.terminalType.name:c instanceof Me?c.nonTerminalName:""}let n=t.name,i=er(e),o=i.idx,s=qr(i),a=r(i),l=o>0,u=`->${s}${l?o:""}<- ${a?`with argument: ->${a}<-`:""}
...
${r}`),this.inline?`{${i}}`:i}};function eG(t,e,r){var n,i;if(t==="linkplain"||t==="linkcode"||t==="link"){let o=e.indexOf(" "),s=e;if(o>0){let l=Iy(e,o);s=e.substring(l),e=e.substring(0,o)}return(t==="linkcode"||t==="link"&&r.link==="code")&&(s=`\`${s}\``),(i=(n=r.renderLink)===null||n===void 0?void 0:n.call(r,e,s))!==null&&i!==void 0?i:tG(e,s)}}function tG(t,e){try{return je.parse(t,!0),`[${e}](${t})`}catch{return t}}var mu=class{constructor(e,r){this.inlines=e,this.range=r}toString(){let e="";for(let r=0;r<this.inlines.length;r++){let n=this.inlines[r],i=this.inlines[r+1];e+=n.toString(),i&&i.range.start.line>n.range.start.line&&(e+=`
...
VAR_INPUT${" ".repeat(8)}${le.variables}${" ".repeat($e)}${W}`,Ce.length>0&&Ce.forEach(q=>{let H=vi(q,I);_+=`
VAR_INPUT${" ".repeat(8)}${q}${" ".repeat(H)}${W}`})}}),d.forEach(B=>{let Q=B.items;for(let ae=0;ae<Q.length;ae++){let le=Q[ae],Ce=le.nextVariables,$e=vi(le.variables,I),W="",L=le.typeName;W=it(W,L),_+=`
VAR_OUTPUT${" ".repeat(7)}${le.variables}${" ".repeat($e)}${W}`,Ce.length>0&&Ce.forEach(q=>{let H=vi(q,I);_+=`
VAR_OUTPUT${" ".repeat(7)}${q}${" ".repeat(H)}${W}`})}});let y={kind:xp.MarkupKind.Markdown,value:["```typescript",_,"```"].join(`
